        ...
        if job.get("metadata"):
            ...
            try:
                ...
                add_metadata_to_image(
                    str(output_path), job_metadata, format=extension.lstrip(".")
                )
                print(
                    f"[Job {job_id}] Added metadata to output image: {output_filename}"
                )
            except Exception as e:
                print(
                    f"[Job {job_id}] Warning: Could not add metadata to image: {str(e)}"
                )
